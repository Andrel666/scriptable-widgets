...
if (teamData && teamData.table) {
    ...
    //leagueTableTapUrl = teamTapUrl.replace("overview", "table");
    leagueTableTapUrl = 
'https://www.fotmob.com/teams/10276/table/vasco-da-gama';
    
    //console.log(teamDataApiUrl);
    console.log(leagueTableTapUrl);
}
...
//3-stack-view
    let paddingLeft = 20;// was 14
    let paddingRight = 3; // was 13
    let paddingTop = 15.5;
    ...
    if (teamData) {
        ...
        // 3-stack-view
            await addWidgetBadge(globalStack);
        //end
    } else {
...
// 3-stack-view
async function addWidgetBadge(globalStack) {
    const badgeStack = globalStack.addStack();
    badgeStack.url = teamTapUrl;
...
async function addWidgetTable(stack) {
  ...
  if(teamData.table) {
    //Fix table data add table[0].data
    let isSingleTable = teamData.table[0].data.table;
    //console.log(teamData.table[0].data);
    //console.log(isSingleTable);
    let leagueTable;
    ...
    if (isSingleTable) {
      ...
      //End Fix table data add table[0].data
    }
    else {
        let teamFound;
        let tableIndex = 0;
